...
Re-ran the §1 forensic check on `outputs/vidigal/morphometrics/grid/grid_metrics.csv`:

- built cells = 2 754; `lambda_f_mean` runs 0–13.1, median 2.78, p5=0.48
- `corr(lambda_f_mean, building_count) = 0.82` (Pearson)
- legacy threshold `λf > 0.35` fires on 97.5% of built cells

Matches the plan-doc table verbatim. The unclipped accumulation in
`src/urban_morphology.py:compute_frontal_area_ratio` is the bug — confirmed
by source read and by the new regression test below.

---

## 1. Prong A — Repair grid λf (cell-clipped)  ✅ VALIDATED
...
**Regression test.** `tests/test_urban_morphology.py::test_clipping_caps_lambda_f_below_unclipped`
...
**Per-site distribution (v2 clipped λf, built cells).**

| Site | n_built | p5 (v1→v2) | p50 (v1→v2) | p95 (v1→v2) | max (v1→v2) | r(λf,bldg_ct) v1→v2 |
|---|---|---|---|---|---|---|
| Vidigal | 2 754 | 0.48 → 0.17 | 2.78 → 1.31 | 6.57 → 3.34 | 13.1 → 5.5 | 0.82 → 0.76 |
| Rocinha | 8 029 | 0.53 → 0.20 | 4.36 → 2.19 | — → 5.01 | 22.3 → 20.0 | — → 0.66 |
| Alemão | 17 766 | 0.35 → 0.14 | 2.49 → 1.15 | — → 3.09 | 23.0 → 9.9 | — → 0.76 |
| Rio das Pedras | 6 605 | 1.17 → 0.33 | 5.62 → 2.69 | — → 5.17 | 24.2 → 10.9 | — → 0.70 |
| Maré | 29 227 | 0.67 → 0.17 | 4.49 → 1.60 | — → 4.33 | 54.6 → 10.7 | — → 0.67 |
...
| Site | 100 m macro p50 | 100 m %>0.40 | 100 m %>0.50 |
|---|---|---|---|
...
Citations correctly attributed: Macdonald, Griffiths & Hall 1998 for WIF→SF;
Oke 1988 only for H/W (not λf); Grimmond & Oke 1999 retained only for bulk
z₀/z_d roughness (not per-cell).

**Outputs:**
- `outputs/<site>/morphometrics/grid/grid_metrics_v2.csv` (5 files)
- `outputs/<site>/morphometrics/grid/grid_metrics_v2.gpkg` (5 files)
- `outputs/brisa_ventilation_fix/lambda_f_v2_stats.json`
...
75–78% skimming (steep, dense), Alemão / Maré at 55–57% (flatter, wider
streets). Differs by 22 pp across sites — i.e. a real signal, unlike the
broken λf which saturated everywhere.
...
cells in each state under **five** ventilation-axis choices.

### Vidigal
| Axis | adequate | sun-only | vent-only | compound |
|---|---|---|---|---|
| **legacy v1** (λf>0.35 broken) | 2.1 | 0.5 | 47.4 | 50.0 |
| v2 λf, Macdonald 0.40 | 9.8 | 3.9 | 39.7 | 46.6 |
| v2 λf, within-site p75 (=2.10) | **41.9** | 33.1 | 7.6 | 17.4 |
...
| Axis | adequate | sun-only | vent-only | compound |
|---|---|---|---|---|
| **legacy v1** | 2.6 | 0.4 | 26.6 | 70.4 |
| v2 λf, Macdonald 0.40 | 7.6 | 2.2 | 21.6 | 68.6 |
| v2 λf, p75 (=3.27) | 27.4 | 47.6 | 1.9 | 23.1 |
...
| Axis | adequate | sun-only | vent-only | compound |
|---|---|---|---|---|
| **legacy v1** | 4.3 | 0.5 | 57.3 | 37.8 |
| v2 λf, Macdonald 0.40 | 15.6 | 2.4 | 46.0 | 36.0 |
| v2 λf, p75 (=1.89) | **52.4** | 22.6 | 9.2 | 15.8 |
...
| Axis | adequate | sun-only | vent-only | compound |
|---|---|---|---|---|
| **legacy v1** | 0.5 | 0.1 | 42.3 | 57.2 |
| v2 λf, Macdonald 0.40 | 5.3 | 0.8 | 37.5 | 56.4 |
| v2 λf, p75 (=3.65) | 36.6 | 38.4 | 6.2 | 18.8 |
...
| Axis | adequate | sun-only | vent-only | compound |
|---|---|---|---|---|
| **legacy v1** | 1.9 | 0.1 | 69.4 | 28.7 |
| v2 λf, Macdonald 0.40 | 13.0 | 0.4 | 58.2 | 28.4 |
| v2 λf, p75 (=2.73) | **60.7** | 14.3 | 10.5 | 14.5 |
| Street SVF < 0.30 | **64.7** | 6.5 | 6.5 | 22.3 |
| H/W > 0.65 | 51.3 | 1.6 | 20.0 | 27.2 |

**What this tells the paper.**

1. The **legacy v1 axis is broken** — across all sites 95–99% of built
   cells fall into vent-only + compound, leaving no resolution.
2. The **clipped v2 λf with the Macdonald threshold is still saturating**
   for the favela density regime — the manuscript must not claim
   Macdonald 0.40 as a within-favela discriminator.
3. The **H/W and street-SVF axes give coherent, discriminating
   results**: across sites the **compound-failure share** under H/W
   correlates very well with under street SVF (Vidigal 45/33, Rocinha
   68/60, Alemão 34/27, Rio das Pedras 55/55, Maré 27/22). These two
   independent geometric channels triangulate the same urban-form story.
4. The **within-site p75 λf axis** is a defensible *relative* threshold
   for spatial mapping — keeps the absolute-threshold honesty in
   §2 while still letting Fig 0.4-style maps identify the worst quartile
   per site.

Outputs: `outputs/brisa_ventilation_fix/taxonomy_v2_stats.json`.
...
|---|---|---|
| A — repair grid λf | ✅ done | Use v2 product (`grid_metrics_v2.csv`); cite Macdonald 1998; **do not** claim Macdonald 0.40 as a within-favela skimming pre-screen. Replace with within-site p75 for spatial maps. |
...
outputs/brisa_ventilation_fix/
  ...
  taxonomy_v2_stats.json

outputs/<site>/morphometrics/
  grid/grid_metrics_v2.{csv,gpkg}  ← Prong A
...
scripts/brisa_deck/
  repair_grid_lambda_f.py
  hw_canyon_proxy.py
  svf_streets_openness.py
  recompute_taxonomy.py
```
