...
| Prep the repo for external review | [`PRODUCTION_READINESS_PLAN.md`](PRODUCTION_READINESS_PLAN.md) |

## Conventions

- **The PDF must be rebuilt in the same commit as a `technical_report.md`
  edit.** A pre-commit hook (`.claude/hooks/check_report_sync.py`) blocks
  commits that violate this.
- **Methodology docs** are reference, not narrative. They define one
  thing each, with formulas and units.
- **Working plans** (e.g. `PRODUCTION_READINESS_PLAN.md`) live here only
  while active; once executed, they're either deleted or absorbed into
