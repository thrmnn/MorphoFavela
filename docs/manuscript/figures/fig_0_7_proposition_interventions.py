#!/usr/bin/env python3
"""Figure 0.7 (proposition A) — From diagnosis to morphological lever.
...
B   Before / after 4-state maps for each site under the **best lever
    per typology** (most recoveries). 2 rows × 5 cols: row 1 = baseline,
    row 2 = after-lever; one column per site.
...
def main() -> None:
    ...
    # Row 1: 2 rows × 5 cols layout — row 1 = baseline, row 2 = after-lever.
    # This avoids the vertical-stretching problem of a 5×2 column layout.
    n_sites = len(SITE_ORDER)
    maps_gs = outer[1].subgridspec(2, n_sites, wspace=0.02, hspace=0.20)
    for j, site in enumerate(SITE_ORDER):
        g = site_grids[site]
        typ = TYPOLOGY_OF[site]
        chosen = best[typ]
        ax_before = fig.add_subplot(maps_gs[0, j])
        ax_after = fig.add_subplot(maps_gs[1, j])
        draw_site_map(ax_before, site, g, buildings[site], boundaries[site],
                      state_col="state_baseline", scalebar=(j == 0),
                      title_suffix="  baseline")
        draw_site_map(ax_after, site, g, buildings[site], boundaries[site],
                      state_col="state_after", scalebar=(j == 0),
    ...
    out_png = EXPORTS_DIR / "fig_0_7_proposition_interventions.png"
    out_svg = EXPORTS_DIR / "fig_0_7_proposition_interventions.svg"
