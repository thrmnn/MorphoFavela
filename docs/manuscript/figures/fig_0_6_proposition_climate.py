#!/usr/bin/env python3
"""Figure 0.6 (proposition C) — Climate stress test.
...
summary), the 4-state diagnosis from Fig 0.4 tilts away from
adequate toward compound failure, but not uniformly: flatland sites
absorb the climate signal first because their adequate cells already
sit closer to the Lawson threshold.

This is a stylized scenario — wind speeds are uniformly scaled by a
factor; no CMIP6 ensemble or thermal coupling. Sunlight is
unaffected by the wind scaling so only the ventilation half of the
...
def main() -> None:
    ...
    for typ in TYPOLOGY_AGGREGATE_ORDER:
        ...
        for s in members:
            ...
            flips = g.apply(
                lambda r: flip_label(r["state_baseline"], r[focal_col]),
                axis=1,
            )
            cls = flips.dropna()
    ...
    # Figure-level title.
    fig.text(0.5, 0.97,
             "Climate stress test: wind stilling tilts flatland sites toward compound failure",
             ha="center", va="top", fontsize=8.5, fontweight="bold",
             color="#1a1a1a")
    fig.text(0.5, 0.95,
             "PROPOSITION C — framework demonstration on synthetic CFD; wind factor is stylized, not CMIP6.",
             ha="center", va="top", fontsize=5.5, style="italic", color="#a0522d")

    out_png = EXPORTS_DIR / "fig_0_6_proposition_climate.png"
    out_svg = EXPORTS_DIR / "fig_0_6_proposition_climate.svg"
