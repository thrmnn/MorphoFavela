...
class TestCPUGPUConsistency:
    ...
    def test_empty_scene_consistency(self):
        ...
        # CPU computation
        cpu_svf = compute_svf(
            test_points, sky_patches, mesh, evaluation_height=1.5, use_gpu=False
        )

        # GPU computation (using unified interface)
        gpu_svf = compute_svf(
            test_points, sky_patches, mesh, evaluation_height=1.5, use_gpu=True
        )
    ...
    def test_single_building_consistency(self):
        ...
        mesh = create_single_building_scene(
            building_size=(10, 10, 20), building_center=(0, 0)
        )
        sky_patches, _ = generate_sky_patches(145)
    ...
    def test_random_points_consistency(self):
        ...
        test_points = generate_test_points_random(
            bounds=(-30, 30, -30, 30, 0, 0), n_points=50, seed=42
        )
...
class TestParameterSensitivity:
    ...
    def test_evaluation_height_consistency(self, height):
        ...
        # CPU computation
        cpu_svf = compute_svf(test_points, sky_patches, mesh, evaluation_height=height)

        # GPU computation
        # GPU computation (using unified interface)
        gpu_svf = compute_svf(
            test_points, sky_patches, mesh, evaluation_height=height, use_gpu=True
        )
