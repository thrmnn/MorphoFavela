...
class TestIntegrateDailyIrradiance:
    ...
    def test_all_sunlit_positive(self):
        ...
        result = integrate_daily_irradiance(
            positions, sunlit, svf=1.0, interval_minutes=60, day_of_year=172
        )
        assert result > 0
    ...
    def test_zero_svf_shaded_zero_irradiance(self):
        ...
        sunlit = np.zeros(1, dtype=bool)
        result = integrate_daily_irradiance(
            positions, sunlit, svf=0.0, interval_minutes=60, day_of_year=172
        )
    ...
    def test_sunlit_greater_than_shaded(self):
        ...
        result_sun = integrate_daily_irradiance(
            positions, sunlit_yes, svf=1.0, interval_minutes=60, day_of_year=172
        )
        result_shade = integrate_daily_irradiance(
            positions, sunlit_no, svf=1.0, interval_minutes=60, day_of_year=172
        )
...
class TestDailyIrradianceArray:
    ...
    def test_consistent_with_scalar_integration(self):
        ...
        result_scalar = integrate_daily_irradiance(
            positions, sunlit_scalar, svf=0.7, interval_minutes=60, day_of_year=172
        )
