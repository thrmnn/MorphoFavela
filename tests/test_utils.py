...
def assert_svf_valid(svf_values, tolerance=1e-6):
    ...
    assert np.all(svf_values >= -tolerance), f"SVF values must be >= 0, found min: {np.min(svf_values)}"
    assert np.all(svf_values <= 1 + tolerance), f"SVF values must be <= 1, found max: {np.max(svf_values)}"
