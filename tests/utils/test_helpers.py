...
def create_single_building_scene(
    ...
    # Building (box)
    building = pv.Box(bounds=(cx - w / 2, cx + w / 2, cy - d / 2, cy + d / 2, 0, h))
...
def generate_test_points_grid(
    bounds: Tuple[float, float, float, float], spacing: float = 5.0, height: float = 0.0
) -> np.ndarray:
    """
...
def generate_test_points_random(
    ...
    points = np.random.uniform(
        low=[x_min, y_min, z_min], high=[x_max, y_max, z_max], size=(n_points, 3)
    )
...
def save_reference_results(test_name: str, svf_values: np.ndarray, output_dir: Path):
...
def assert_reference_match(
    current_svf: np.ndarray, reference_results: dict, tolerance: float = 0.01
):
