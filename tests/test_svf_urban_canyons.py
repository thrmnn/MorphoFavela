...
def create_street_canyon(
    ...
    building_left = pv.Box(
        bounds=(
            cx - half_width - 10,
            cx - half_width,  # x bounds
            cy - length / 2,
            cy + length / 2,  # y bounds
            0,
            building_height,  # z bounds
        )
    )
...
def calculate_expected_svf_street_canyon(
    width: float, height: float, observer_height: float = 1.5
) -> float:
    """
...
def create_analytical_canyon(
    width: float, building_height: float, length: float = 200.0
) -> pv.PolyData:
    """
    ...
    building_left = pv.Box(
        bounds=(
            -half_width - 0.1,
            -half_width,  # x bounds (thin wall)
            -length / 2,
            length / 2,  # y bounds (long)
            0,
            building_height,  # z bounds
        )
    )
    ...
    building_right = pv.Box(
        bounds=(
            half_width,
            half_width + 0.1,  # x bounds (thin wall)
            -length / 2,
            length / 2,  # y bounds (long)
            0,
            building_height,  # z bounds
...
def calculate_analytical_svf(
    width: float, building_height: float, observer_height: float = 1.5
) -> float:
    """
...
class TestStreetCanyonNarrow:
    ...
    def test_narrow_canyon_center_cpu(self):
        ...
        assert svf_values[0] < 0.3, (
            f"Expected low SVF in narrow canyon, got {svf_values[0]:.3f}"
        )

    @pytest.mark.skipif(
        not GPU_AVAILABLE or not torch.cuda.is_available(), reason="CUDA not available"
    )
...
class TestStreetCanyonWide:
    ...
    def test_wide_canyon_center_cpu(self):
        ...
        assert svf_values[0] > 0.5, (
            f"Expected higher SVF in wide canyon, got {svf_values[0]:.3f}"
        )
...
class TestStreetCanyonCPUvsGPU:
    ...
    def test_canyon_consistency(self, width, height):
        ...
        stats = compare_cpu_gpu_results(
            cpu_svf, gpu_svf, abs_tolerance=0.03, max_abs_tolerance=0.1
        )
...
class TestStreetCanyonKnownValues:
    ...
    def test_very_wide_canyon(self):
        ...
        assert svf_values[0] > 0.8, (
            f"Expected high SVF in very wide canyon, got {svf_values[0]:.3f}"
        )
    ...
    def test_very_narrow_canyon(self):
        ...
        assert_svf_valid(svf_values)
        # Very narrow canyon should have very low SVF
        assert svf_values[0] < 0.2, (
            f"Expected very low SVF in narrow canyon, got {svf_values[0]:.3f}"
        )
...
def calculate_analytical_svf_at_position(
    x: float, width: float, building_height: float, observer_height: float = 1.5
) -> float:
    """
...
class TestSVFInterpolation:
    ...
    def test_linear_interpolation_moderate_canyon(self):
        ...
        sampling_points = np.array(
            [[x, 0.0, 0.0] for x in sampling_x], dtype=np.float64
        )
        svf_sampled = compute_svf(
            sampling_points, sky_patches, mesh, evaluation_height=h_obs
        )
