...
class TestMeshConversion:
    ...
    def test_pv_to_pytorch3d_conversion(self):
        ...
        assert torch.all(faces < len(verts)), (
            "Face indices should be within vertex range"
        )
...
class TestObserverPointPreparation:
    ...
    def test_height_offset(self):
        ...
        ground_points = np.array([[0, 0, 0], [1, 1, 5], [2, 2, 10]], dtype=np.float32)
        evaluation_height = 1.5

        observer_points = prepare_observer_points(
            ground_points, evaluation_height, device=torch.device("cpu")
        )
    ...
    def test_tensor_format(self):
        ...
        observer_points = prepare_observer_points(
            ground_points, 1.5, device=torch.device("cpu")
        )
    ...
    def test_device_consistency(self):
        ...
        if torch.cuda.is_available():
            observer_points_gpu = prepare_observer_points(
                ground_points, 1.5, device=torch.device("cuda")
            )
...
class TestSkyPatchPreparation:
    ...
    def test_tensor_conversion(self):
        ...
        assert torch.allclose(
            patches_tensor, torch.tensor(sky_patches, dtype=torch.float32)
