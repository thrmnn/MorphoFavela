...
def georeference_stl_using_dtm(
    ...
    stl_x_min, stl_x_max, stl_y_min, stl_y_max, stl_z_min, stl_z_max = (
        stl_bounds[0], stl_bounds[1], stl_bounds[2], stl_bounds[3], stl_bounds[4], stl_bounds[5]
    )
    ...
    logger.info(f"  STL bounds (local): ({stl_x_min:.2f}, {stl_y_min:.2f}, {stl_z_min:.2f}) to ({stl_x_max:.2f}, {stl_y_max:.2f}, {stl_z_max:.2f})")
    logger.info(f"  STL center (local): ({stl_center_x:.2f}, {stl_center_y:.2f}, {stl_center_z:.2f})")
...
def validate_alignment(
    ...
    if reference_bounds is None and roads_gdf is not None and len(roads_gdf) > 0:
        ...
        reference_bounds = (road_bounds[0], road_bounds[1], road_bounds[2], road_bounds[3])
    ...
    stl_x_min, stl_x_max, stl_y_min, stl_y_max = stl_bounds[0], stl_bounds[1], stl_bounds[2], stl_bounds[3]
    ref_x_min, ref_y_min, ref_x_max, ref_y_max = reference_bounds[0], reference_bounds[1], reference_bounds[2], reference_bounds[3]
    ...
    logger.info(f"Alignment validated: center difference X={center_diff_x:.3f}m, Y={center_diff_y:.3f}m")
...
def redirect_road_parallel_offset(
    ...
    try:
        ...
        combined_buffer = unary_union(building_buffers.values) if len(building_buffers) > 0 else None
...
def redirect_road_simple_reroute(
    ...
    buffer_distances = [buffer_distance, buffer_distance * 1.5, buffer_distance * 2.0, buffer_distance * 3.0, buffer_distance * 4.0]
        ...
        try:
            ...
            if len(segments) > 1:
                ...
                try:
                    ...
                    segments_sorted = sorted(segments, key=lambda s: Point(s.coords[0]).distance(road_start))
...
def redirect_roads(
    ...
    if successfully_redirected_indices:
        ...
        redirected_info = gpd.GeoDataFrame(columns=intersection_info.columns, crs=intersection_info.crs)
