...
def pv_mesh_to_pytorch3d(
    pv_mesh: pv.PolyData, device: Optional[torch.device] = None
) -> Meshes:
    """
    ...
    if not valid_mask.all():
        ...
        print(
            f"  Warning: Found {n_invalid} vertices with NaN values, filtering them out"
        )
        ...
        valid_faces_mask = (
            valid_mask[faces_old].all(axis=1)
            if len(faces_old) > 0
...
def prepare_sky_patches(
    sky_patches: np.ndarray, device: Optional[torch.device] = None
) -> torch.Tensor:
    """
