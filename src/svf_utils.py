...
def filter_buildings(footprints: gpd.GeoDataFrame, area: str = None) -> tuple:
    ...
    if len(footprints) == 0:
        isolated = gpd.GeoDataFrame(
            geometry=[],
            crs=footprints.crs if len(footprints) > 0 else area_filtered.crs,
        )
        return footprints, isolated, area_filtered
    ...
    if initial_count != final_count:
        ...
        print(
            f"  Total filtered: {initial_count} → {final_count} ({100 * total_removed / initial_count:.1f}% removed)"
        )
        if isolated_count > 0:
...
def load_building_footprints(
    ...
    footprints, isolated_footprints, area_filtered_footprints = filter_buildings(
    ...
    if len(area_filtered_footprints) > 0:
        area_filtered_footprints.geometry = area_filtered_footprints.geometry.translate(
            xoff=dx, yoff=dy
        )
        # Ensure CRS matches for plotting
...
def compute_ground_mask(
    grid_x: np.ndarray, grid_y: np.ndarray, building_footprints: gpd.GeoDataFrame
) -> np.ndarray:
    """
...
def plot_ground_mask_debug(
    ...
    if building_buffer_geom is not None:
        ...
        buffer_gdf.plot(
            ...
            alpha=0.3,
            linestyle="--",
            label=f"Proximity buffer ({building_buffer}m)",
        )
    ...
    if len(ground_mask) == len(grid_x):
        ...
        ax.scatter(
            ...
            s=3,
            alpha=0.7,
            label="Ground points (SVF will be computed)",
...
def generate_ground_points(
    ...
    if ground_mask is not None:
        if len(ground_mask) != len(grid_points_2d):
            print(
                f"  Warning: Mask size ({len(ground_mask)}) doesn't match grid size ({len(grid_points_2d)})"
            )
        ...
        if len(ground_mask) == len(grid_points_2d):
            print(
                f"  Applying ground mask: {np.sum(ground_mask)}/{len(ground_mask)} points are ground"
            )
    ...
    if (
        ...
        print(
            f"  Points near buildings: {num_near_buildings}/{len(grid_points_2d)} ({100 * num_near_buildings / len(grid_points_2d):.1f}%)"
        )
