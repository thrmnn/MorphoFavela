...
def generate_analysis_units(
    footprints: gpd.GeoDataFrame, grid_size: float = 50.0
) -> gpd.GeoDataFrame:
    """
...
def aggregate_by_unit(
    footprints: gpd.GeoDataFrame, analysis_units: gpd.GeoDataFrame
) -> gpd.GeoDataFrame:
    """
...
def compute_open_space(
    analysis_units: gpd.GeoDataFrame, footprints: gpd.GeoDataFrame
) -> gpd.GeoDataFrame:
    """
...
def compute_density_proxy(
    analysis_units: gpd.GeoDataFrame, min_open_space: float = 1.0
) -> gpd.GeoDataFrame:
    """
    ...
    if zero_open_mask.any():
        logger.warning(
            f"  {zero_open_mask.sum()} units have insufficient open space (<{min_open_space}m²)"
        )
    ...
    logger.info(
        f"  Mean density proxy: {result[result['density_proxy'] != np.inf]['density_proxy'].mean():.2f} m³/m²"
    )
    logger.info(
        f"  Max density proxy: {result[result['density_proxy'] != np.inf]['density_proxy'].max():.2f} m³/m²"
    )

    return result


def plot_density_map(analysis_units: gpd.GeoDataFrame, output_path: Path) -> None:
    """
    ...
    if len(finite_units) == 0:
        ...
        finite_units.plot(
            ...
            vmin=vmin,
            vmax=vmax,
            legend_kwds={"label": "Density Proxy (m³/m²)", "shrink": 0.8},
        )
...
It does NOT represent population. It is used only for relative ranking.
    ...
    parser.add_argument(
        "--stl", type=str, required=True, help="Path to STL file (terrain + buildings)"
    )
    parser.add_argument(
        "--footprints",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to analysis units shapefile (blocks/clusters). If not provided, a regular grid will be generated.",
    )
    parser.add_argument(
        "--grid-size",
        type=float,
        default=50.0,
        help="Grid cell size in meters for auto-generated units (default: 50.0)",
    )
    parser.add_argument(
        "--z-threshold",
        type=float,
        default=None,
        help="Z threshold to separate terrain from buildings",
    )
    parser.add_argument(
        "--min-open-space",
        type=float,
        default=1.0,
        help="Minimum open space threshold (m²) to avoid division by zero",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default=None,
        help="Output directory (default: outputs/density)",
    )
    ...
    footprints_with_volume = compute_building_volumes(
        full_mesh, footprints, full_mesh.bounds, z_threshold=args.z_threshold
    )
