...
def compute_pixel_level_occupancy(
    ...
    volumes = []
    for idx, row in tqdm(
        footprints_with_volume.iterrows(),
        total=len(footprints_with_volume),
        ...
        for p in mesh_points:
            x, y, z = p
            if (
                bounds_2d[0] <= x <= bounds_2d[2]
                and bounds_2d[1] <= y <= bounds_2d[3]
                and min_z <= z <= max_z
                and geom.contains(Point(x, y))
            ):
    ...
    for i, y in enumerate(tqdm(grid_y_coords, desc="  Processing rows")):
        for j, x in enumerate(grid_x_coords):
            ...
            buildings_in_cell = footprints_with_volume[
                footprints_with_volume.geometry.intersects(cell)
            ]
    ...
    logger.info(
        f"  Mean occupancy pressure: {np.nanmean(occupancy_pressure[~np.isinf(occupancy_pressure)]):.2f} m³/m²"
    )
...
def compute_raster_deficits(
    ...
    valid_occupancy = occupancy_map[
        ~np.isnan(occupancy_map) & ~np.isinf(occupancy_map) & (occupancy_map >= 0)
    ]
...
def apply_building_mask(
    ...
    logger.info(
        f"  Masked {building_mask.sum()} building pixels ({building_mask.sum() / building_mask.size * 100:.1f}%)"
    )

    return masked_index


def classify_hotspot_raster(hotspot_index: np.ndarray) -> np.ndarray:
    """
...
def plot_continuous_raster(
    ...
    ax.set_title(
        "Environmental Deprivation Index (Continuous Raster)\nHigher values = Greater deprivation",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_aspect("equal")
...
def plot_classified_raster(
    ...
    ax.set_ylabel("Y (meters)", fontsize=12)
    ax.set_title(
        "Environmental Deprivation Hotspots (Classified)\nTop 10%: Extreme, 10-20%: High, Remaining: Baseline",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_aspect("equal")
...
def plot_unit_level_map(analysis_units: gpd.GeoDataFrame, output_path: Path) -> None:
    """
    ...
    analysis_units.plot(
        ...
        vmin=0.0,
        vmax=1.0,
        legend_kwds={"label": "Mean Hotspot Index (0-1)", "shrink": 0.8},
    )

    ax.set_xlabel("X (meters)", fontsize=12)
    ax.set_ylabel("Y (meters)", fontsize=12)
    ax.set_title(
        "Environmental Deprivation Index (Unit-Level Aggregation)",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_aspect("equal")
...
All metrics are relative and distribution-based. No causality is inferred.
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to STL file (for volume computation)",
    )
    parser.add_argument(
        "--footprints",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional path to analysis units for aggregation",
    )
    parser.add_argument(
        "--solar-reference",
        type=float,
        default=None,
        help="Reference solar hours (default: median)",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default=None,
        help="Output directory (default: outputs/deprivation_raster)",
    )
    ...
    logger.info(
        f"Raster shapes - Solar: {solar_map.shape}, SVF: {svf_map.shape}, Porosity: {porosity_map.shape}"
    )
    ...
    plot_classified_raster(
        classified, grid_x_coords, grid_y_coords, output_dir / "hotspot_classified.png"
    )
