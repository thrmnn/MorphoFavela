...
def compute_deficits(
    analysis_units: gpd.GeoDataFrame, solar_reference: float = None
) -> gpd.GeoDataFrame:
    """
...
def compute_hotspot_index(analysis_units: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """
...
def classify_hotspots(analysis_units: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    """
    ...
    result.loc[
        (result["hotspot_index"] >= p80) & (result["hotspot_index"] < p90),
        "hotspot_class",
    ] = "High deprivation"
...
def plot_hotspot_map(analysis_units: gpd.GeoDataFrame, output_path: Path) -> None:
    """
    ...
    for class_name, color in color_map.items():
        ...
        if len(class_units) > 0:
            patch = class_units.plot(
                ax=ax, color=color, edgecolor="black", linewidth=0.5, label=class_name
            )
...
def export_ranking_table(analysis_units: gpd.GeoDataFrame, output_path: Path) -> None:
    """
    ...
    result["occupancy_percentile"] = (
        result["occupancy_score"].rank(method="average", pct=True) * 100
    )
...
All metrics are relative and distribution-based. No causality is inferred.
    ...
    parser.add_argument(
        "--units", type=str, required=True, help="Path to analysis units shapefile/GPKG"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to density proxy GeoDataFrame (.gpkg or .shp)",
    )
    parser.add_argument(
        "--solar-reference",
        type=float,
        default=None,
        help="Reference solar hours (default: median of data)",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default=None,
        help="Output directory (default: outputs/deprivation)",
    )
