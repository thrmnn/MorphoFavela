...
def sample_street_points(
    roads_gdf: gpd.GeoDataFrame, spacing: float, terrain: pv.PolyData = None
) -> gpd.GeoDataFrame:
    """
...
def aggregate_segment_statistics(
    points_gdf: gpd.GeoDataFrame, solar_hours: np.ndarray, roads_gdf: gpd.GeoDataFrame
) -> gpd.GeoDataFrame:
    """
...
def create_street_solar_map(
    ...
    ax.set_title(
        "Street-Level Solar Access (Winter Solstice)\nColored by Mean Hours per Segment",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_aspect("equal")
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to building footprints (for visualization)",
    )
    parser.add_argument(
        "--spacing",
        type=float,
        default=3.0,
        help="Point spacing along streets (meters)",
    )
    parser.add_argument(
        "--height",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=-22.9519,
        help="Site latitude (default: Rio de Janeiro)",
    )
    parser.add_argument(
        "--longitude",
        type=float,
        default=-43.2105,
        help="Site longitude (default: Rio de Janeiro)",
    )
    ...
    sun_directions, sun_times = compute_sun_positions(
        latitude=args.latitude, longitude=args.longitude, timestep_minutes=60
    )
    ...
    create_solar_distribution_plot(
        points_gdf, segments_gdf, output_dir / "street_solar_distribution.png"
    )
