...
def create_deprivation_distribution_plots(
    segments_gdf: gpd.GeoDataFrame, points_gdf: gpd.GeoDataFrame, output_dir: Path
):
    """
    ...
    ax.axvline(
        ...
        linewidth=1.5,
        alpha=0.7,
        label="Extreme deprivation (>0.75)",
    )
...
def main():
    ...
    if "segment_idx" in deprivation_points.columns:
        segments = (
            deprivation_points.groupby("segment_idx")
            .agg(
                {
    ...
    if building_file is not None and segments is not None:
        try:
            ...
            building_footprints = load_building_footprints(
                building_file, terrain_bounds=bounds, buffer_distance=0.0
            )
            # Ensure same CRS
    ...
    if segments is not None and isinstance(segments, gpd.GeoDataFrame):
        ...
        logger.warning(
            "  Cannot create distribution plots (segments not available as GeoDataFrame)"
        )
