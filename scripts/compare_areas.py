...
def load_area_data(area: str) -> dict:
    ...
    data = {"area": area, "area_type": "formal" if is_formal_area(area) else "informal"}
    ...
    for raster_type in ["svf", "solar", "porosity", "deprivation_raster"]:
        ...
        raster_file = (
            raster_dir
            / f"{'solar_access' if raster_type == 'solar' else 'svf' if raster_type == 'svf' else 'porosity' if raster_type == 'porosity' else 'hotspot_index'}.npy"
        )
        ...
        if raster_file.exists():
            ...
            data[f"{raster_type}_valid_fraction"] = (
                valid.sum() / raster.size if raster.size > 0 else 0
            )
    ...
    for metric in metrics:
        if vidigal_data["buildings"] is not None and metric in vidigal_data["buildings"].columns:
            ...
            copa_vals = (
                ...
                if copacabana_data["buildings"] is not None
                and metric in copacabana_data["buildings"].columns
                else pd.Series()
            )
            ...
            if len(vidigal_vals) > 0 and len(copa_vals) > 0:
                row = {
                    ...
                    "ratio_mean": copa_vals.mean() / vidigal_vals.mean()
                    if vidigal_vals.mean() > 0
                    else np.nan,
                }
                ...
                try:
                    stat, p_value = stats.mannwhitneyu(
                        vidigal_vals, copa_vals, alternative="two-sided"
                    )
                    row["p_value"] = p_value
...
def compare_street_metrics(vidigal_data: dict, copacabana_data: dict, street_type: str) -> dict:
    ...
    vid_total_length = (
        vid_segments["length"].sum()
        if vid_segments is not None and "length" in vid_segments.columns
        else np.nan
    )
    copa_total_length = (
        copa_segments["length"].sum()
        if copa_segments is not None and "length" in copa_segments.columns
        else np.nan
    )
    ...
    comparison = {
        ...
        "vidigal_points_per_km": (len(vid_vals) / vid_total_length * 1000)
        if not np.isnan(vid_total_length) and vid_total_length > 0
        else np.nan,
        ...
        "copa_points_per_km": (len(copa_vals) / copa_total_length * 1000)
        if not np.isnan(copa_total_length) and copa_total_length > 0
        else np.nan,
        "copa_mean": copa_vals.mean(),
...
def create_comparison_visualizations(
    ...
    fig.suptitle(
        "Morphometric Metrics: Distribution Comparison",
        fontsize=14,
        fontweight="bold",
        color=SWISS_COLORS["text"],
        y=0.995,
    )
        ...
        if vidigal_data["buildings"] is not None and metric in vidigal_data["buildings"].columns:
            ...
            if len(vid_vals) > 0 and len(copa_vals) > 0:
                ...
                ax.hist(
                    ...
                    bins=bins,
                    alpha=0.7,
                    label="Vidigal (Informal)",
                    color=SWISS_COLORS["vidigal_tls"],
                    density=True,
                    edgecolor="white",
                    linewidth=0.5,
                )
                ax.hist(
                    ...
                    bins=bins,
                    alpha=0.7,
                    label="Copacabana (Formal)",
                    color=SWISS_COLORS["copacabana"],
                    density=True,
                    edgecolor="white",
                    linewidth=0.5,
                )
                ...
                ax.legend(
                    ...
                    frameon=True,
                    fancybox=False,
                    edgecolor=SWISS_COLORS["secondary"],
                    framealpha=0.9,
                )
    ...
    fig.patch.set_facecolor("white")
    fig.suptitle(
        "Morphometric Metrics: Box Plot Comparison",
        fontsize=14,
        fontweight="bold",
        color=SWISS_COLORS["text"],
        y=0.995,
    ...
    if data_to_plot:
        ...
        bp1 = ax.boxplot(
            [d[0] for d in data_to_plot],
            positions=positions - 0.4,
            widths=0.35,
            patch_artist=True,
            tick_labels=[l + "\n(Vidigal)" for l in labels],
            boxprops=dict(
                facecolor=SWISS_COLORS["vidigal_tls"],
                alpha=0.8,
                edgecolor=SWISS_COLORS["text"],
                linewidth=0.8,
            ),
        ...
        bp2 = ax.boxplot(
            [d[1] for d in data_to_plot],
            positions=positions + 0.4,
            widths=0.35,
            patch_artist=True,
            tick_labels=[l + "\n(Copa)" for l in labels],
            boxprops=dict(
                facecolor=SWISS_COLORS["copacabana"],
                alpha=0.8,
                edgecolor=SWISS_COLORS["text"],
                linewidth=0.8,
            ),
    ...
    for raster_type, title in raster_types.items():
        ...
        if vid_raster is not None and copa_raster is not None:
            ...
            fig.patch.set_facecolor("white")
            fig.suptitle(
                f"{title}: Side-by-Side Comparison",
                fontsize=14,
                fontweight="bold",
                color=SWISS_COLORS["text"],
                y=0.98,
            ...
            gs = GridSpec(
                ...
                2,
                figure=fig,
                width_ratios=[vid_aspect, copa_aspect],
                wspace=0.3,
                left=0.05,
                right=0.95,
                top=0.92,
    ...
    if vid_svf_map_path.exists() and copa_svf_map_path.exists():
        ...
        fig.patch.set_facecolor("white")
        fig.suptitle(
            "Street-Level Sky View Factor (SVF): Side-by-Side Comparison",
            fontsize=14,
            fontweight="bold",
            color=SWISS_COLORS["text"],
            y=0.98,
    ...
    if vid_solar_map_path.exists() and copa_solar_map_path.exists():
        ...
        fig.patch.set_facecolor("white")
        fig.suptitle(
            "Street-Level Solar Access: Side-by-Side Comparison",
            fontsize=14,
            fontweight="bold",
            color=SWISS_COLORS["text"],
            y=0.98,
    ...
    if vid_dep_segments is None or copa_dep_segments is None:
        ...
        if (
            vid_dep_points is not None
            and copa_dep_points is not None
            and "deprivation_index" in vid_dep_points.columns
    ...
    if (
        ...
        and copa_dep_segments is not None
        and "deprivation_mean" in vid_dep_segments.columns
        and "deprivation_mean" in copa_dep_segments.columns
    ):
        ...
        if len(vid_vals) > 0 and len(copa_vals) > 0:
            ...
            fig.patch.set_facecolor("white")
            fig.suptitle(
                "Street-Level Deprivation Index: Side-by-Side Comparison",
                fontsize=14,
                fontweight="bold",
                color=SWISS_COLORS["text"],
                y=0.98,
            ...
            if vid_buildings is not None:
                vid_buildings_plot = (
                    vid_buildings.to_crs(vid_dep_segments.crs)
                    if vid_buildings.crs != vid_dep_segments.crs
                    else vid_buildings
                )
                vid_buildings_plot.plot(
                    ax=ax1,
                    facecolor="lightgrey",
                    edgecolor="black",
                    linewidth=0.3,
            ...
            vid_dep_segments.plot(
                ax=ax1,
                column="deprivation_mean",
                cmap=dep_cmap,
                vmin=vmin,
                vmax=vmax,
                linewidth=2,
                legend=True,
                legend_kwds={"label": "Deprivation Index", "shrink": 0.8},
                zorder=1,
            )
            ax1.set_title(
            ...
            if copa_buildings is not None:
                copa_buildings_plot = (
                    copa_buildings.to_crs(copa_dep_segments.crs)
                    if copa_buildings.crs != copa_dep_segments.crs
                    else copa_buildings
                )
                copa_buildings_plot.plot(
                    ax=ax2,
                    facecolor="lightgrey",
                    edgecolor="black",
                    linewidth=0.3,
        ...
        if vid_points is not None and copa_points is not None:
            ...
            if value_col in vid_points.columns and value_col in copa_points.columns:
                ...
                if len(vid_vals) > 0 and len(copa_vals) > 0:
                    ...
                    fig.patch.set_facecolor("white")
                    fig.suptitle(
                        f"Street-Level {title}: Distribution Comparison",
                        fontsize=14,
                        fontweight="bold",
                        color=SWISS_COLORS["text"],
                        y=0.95,
                    ...
                    fig.patch.set_facecolor("white")
                    fig.suptitle(
                        f"Street-Level {title}: Box Plot Comparison",
                        fontsize=14,
                        fontweight="bold",
                        color=SWISS_COLORS["text"],
                        y=0.95,
    ...
    if (
        vid_dep is not None
        and copa_dep is not None
        and "deprivation_index" in vid_dep.columns
        and "deprivation_index" in copa_dep.columns
    ):
        vid_vals = vid_dep["deprivation_index"].dropna().values
        copa_vals = copa_dep["deprivation_index"].dropna().values
        ...
        if len(vid_vals) > 0 and len(copa_vals) > 0:
            ...
            fig.patch.set_facecolor("white")
            fig.suptitle(
                "Street-Level Deprivation Index: Distribution Comparison",
                fontsize=14,
                fontweight="bold",
                color=SWISS_COLORS["text"],
                y=0.95,
            ...
            ax.set_xlim(min_val, max_val)
            ax.set_xlabel(
                "Deprivation Index (0-1, higher = more deprived)",
                fontsize=10,
                color=SWISS_COLORS["text"],
            )
            ax.set_ylabel("Density", fontsize=10, color=SWISS_COLORS["text"])
...
def generate_pdf_report(
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        fig.text(
            0.5,
            0.58,
            "Vidigal (Informal)  ×  Copacabana (Formal)",
            ha="center",
            va="center",
            fontsize=14,
            color=SWISS_COLORS["text"],
        ...
        fig.text(
            0.5,
            0.1,
            "Morphometric & Environmental Performance Analysis",
            ha="center",
            va="center",
            fontsize=10,
            color=SWISS_COLORS["secondary"],
        ...
        ax.text(
            0.15,
            current_y,
            f"Buildings: {vidigal_data['building_count']}",
            transform=ax.transAxes,
            fontsize=9,
            color=SWISS_COLORS["text"],
            family="sans-serif",
        ...
        ax.text(
            0.15,
            current_y,
            f"Study Area: {vidigal_data['study_area_m2'] / 1e6:.2f} km²",
            transform=ax.transAxes,
            fontsize=9,
            color=SWISS_COLORS["text"],
            family="sans-serif",
        ...
        ax.text(
            0.15,
            current_y,
            f"Building Density: {vidigal_data['building_density']:.1f} buildings/km²",
            transform=ax.transAxes,
            fontsize=9,
            color=SWISS_COLORS["text"],
            family="sans-serif",
        ...
        ax.text(
            0.15,
            current_y,
            f"Buildings: {copacabana_data['building_count']}",
            transform=ax.transAxes,
            fontsize=9,
            color=SWISS_COLORS["text"],
            family="sans-serif",
        ...
        ax.text(
            0.15,
            current_y,
            f"Study Area: {copacabana_data['study_area_m2'] / 1e6:.2f} km²",
            transform=ax.transAxes,
            fontsize=9,
            color=SWISS_COLORS["text"],
            family="sans-serif",
        ...
        ax.text(
            0.15,
            current_y,
            f"Building Density: {copacabana_data['building_density']:.1f} buildings/km²",
            transform=ax.transAxes,
            fontsize=9,
            color=SWISS_COLORS["text"],
            family="sans-serif",
        ...
        if street_comparisons:
            ax.text(
                0.1,
                current_y,
                "STREET-LEVEL PERFORMANCE (Primary Focus):",
                transform=ax.transAxes,
                fontsize=12,
                fontweight="bold",
                color=SWISS_COLORS["accent"],
            ...
            for street_type, comp in street_comparisons.items():
                if comp:
                    ...
                    ax.text(
                        0.2,
                        current_y,
                        f"Vidigal={comp['vidigal_mean']:.3f}{unit}, Copacabana={comp['copa_mean']:.3f}{unit}",
                        transform=ax.transAxes,
                        fontsize=9,
                        color=SWISS_COLORS["text"],
                        family="sans-serif",
                    ...
                    ax.text(
                        0.2,
                        current_y,
                        f"Effect: {effect_str} (d={comp.get('effect_size', 0):.2f}){sig_str}",
                        transform=ax.transAxes,
                        fontsize=9,
                        color=SWISS_COLORS["accent"]
                        if comp.get("significant", False)
                        else SWISS_COLORS["secondary"],
                        family="sans-serif",
                        style="italic",
                        verticalalignment="top",
                    )
        ...
        key_figs = [
            "svf_streets_side_by_side",
            "solar_streets_side_by_side",
            "svf_streets_distribution",
            "solar_streets_distribution",
            "deprivation_streets_distribution",
            "svf_streets_boxplot",
            "solar_streets_boxplot",
        ]
        ...
        if street_comparisons:
            ...
            ax.text(
                0.5,
                0.97,
                "Street-Level Performance Comparison",
                ha="center",
                va="top",
                fontsize=18,
                fontweight="bold",
                ...
                if comp:
                    ...
                    street_stats.append(
                        [
                            ...
                            f"{comp.get('effect_size', np.nan):.2f}"
                            if "effect_size" in comp
                            and not np.isnan(comp.get("effect_size", np.nan))
                            else "N/A",
                            effect_str,
                            sig_str,
                        ]
        ...
        if raster_comparisons:
            ...
            ax.text(
                0.5,
                0.97,
                "Environmental Performance Metrics Comparison",
                ha="center",
                va="top",
                fontsize=16,
                fontweight="bold",
            ...
            for raster_type in ["svf", "solar"]:
                ...
                if comp:
                    name = {
                        "svf": "Sky View Factor (Grid)",
                        "solar": "Solar Access (Grid, hours)",
                    }[raster_type]
                    effect_str = comp.get("effect_interpretation", "N/A")
                    sig_str = "✓" if comp.get("significant", False) else ""
                    raster_stats.append(
...
def compare_zone_metrics(area1_data: dict, area2_data: dict) -> list[dict] | None:
    ...
    logger.info(
        "Loaded zone metrics: %s (%d zones), %s (%d zones)",
        area1_name,
        len(gdf1),
        area2_name,
        len(gdf2),
...
def compare_moran_summary(area1_data: dict, area2_data: dict) -> pd.DataFrame | None:
    ...
    logger.info(
        "Combined Moran summary: %d rows from %s and %s.",
        len(combined),
        area1_name,
        area2_name,
    )
...
def main():
    ...
    morpho_comparison.to_csv(
        output_dir / "tables" / "comparison_morphometric_stats.csv", index=False
    )
    logger.info(
        f"Saved morphometric comparison to {output_dir / 'tables' / 'comparison_morphometric_stats.csv'}"
    )
    ...
    try:
        ...
        if cluster_result is not None:
            ...
            logger.info(
                "Saved cluster composition to %s",
                output_dir / "tables" / "comparison_cluster_composition.csv",
            )
        else:
    ...
    figs = create_comparison_visualizations(
        vidigal_data, copacabana_data, morpho_comparison, raster_comparisons, output_dir
    )
