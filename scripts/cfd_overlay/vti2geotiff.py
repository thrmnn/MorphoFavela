...
# ----------------------------------------------------------------------
def local_to_world(xy: np.ndarray, theta_ccw: float,
                   offset_xy: tuple) -> np.ndarray:
...
# ----------------------------------------------------------------------
def extract_layer(vti, theta_ccw, offset_xy, terrain, dz_datum,
                  slab_agl, reduce):
    ...
    if "tau" in vti.point_data:
        ...
        tau = np.asarray(vti.cell_data_to_point_data()
                         .point_data["tau"], float)
    valid = vti.point_data.get("vtkValidPointMask")
    solid = (np.asarray(valid) == 0) if valid is not None \
        else ~np.isfinite(tau)
...
def wind_weights(rose_path: Path, dirs, allow_placeholder):
    ...
    if raw.sum() <= 0:
        raise SystemExit("FAIL-CLOSED: wind-rose weights sum to 0 over "
                         f"available directions {dirs}.")
...
def write_geotiff(path, arr, transform, epsg, nodata, tags):
    ...
    with rasterio.open(
        ...
        dtype="float32", crs=f"EPSG:{epsg}", nodata=nodata,
...
def run_self_test():
    ...
    for d, wdeg in WIND8.items():
        ...
        for theta in (math.radians((90 + wdeg) % 360), 0.0):
            ...
            if not ok:
                bad += 1
            print(f"  {tag} dir={d:2s} θ={math.degrees(theta):6.1f}°  "
    ...
    print(f"\nSELF-TEST {'PASSED' if ok_all else 'FAILED'} "
          f"({bad} bad placements)")
...
def main():
    ...
    ap.add_argument("--smoke-vti", type=Path,
                    help="single VTI for --dry-run before the 8-dir run")
    ap.add_argument("--case-meta", type=Path,
                    help="case_meta.json paired with --smoke-vti")
    ap.add_argument("--secondary", action="store_true",
                    help="also emit the canopy [z0,H_mean] vol-mean band")
    ...
    ap.add_argument("--self-test", action="store_true",
                    help="synthetic round-trip only; no CFD input needed")
    a = ap.parse_args()
    ...
    if terrain.crs_epsg != 31983:
        raise SystemExit(f"FAIL-CLOSED: terrain EPSG {terrain.crs_epsg} "
                         "!= 31983 (verdict §9-iv axis/CRS).")
    ...
    b = (cx - ANALYSIS_RADIUS_M - HALO_M, cy - ANALYSIS_RADIUS_M - HALO_M,
         cx + ANALYSIS_RADIUS_M + HALO_M, cy + ANALYSIS_RADIUS_M + HALO_M)
    ...
    spec = GeorefSpec(
        patch_id=a.patch, crs="EPSG:31983",
        ...
        composite_rule="ACH_i=3600/tau_i per cell; ACH_comp=Σ f_i ACH_i; "
                       "tau_comp=1/ACH_comp",
        ...
        units_tau="s (age of air)", units_ach="1/h",
        nodata=nodata, contract=CONTRACT, contract_ok=True,
    ...
    print(f"== gate: synthetic round-trip (composite frame) "
    ...
    for d, wdeg, vti_p, cm_p in srcs:
        ...
        xy, tau = extract_layer(vti, th, off, terrain, dz,
                                (PED_LO_AGL, PED_HI_AGL), reduce="none")
        tau_grid = resample(xy, tau, transform, shape, np.nan)
        ach = np.where(np.isfinite(tau_grid) & (tau_grid > 0),
                       3600.0 / tau_grid, np.nan)
        ...
        print(f"  {d:5s} θ={math.degrees(th):6.1f}° w={wd:.3f} "
              f"contrib_px={int(contrib.sum())}")
    ...
    tau_comp[good] = (1.0 / (ach_acc[good] / wsum[good]) * 3600.0
                      ).astype(np.float32)
    ...
    if np.isfinite(tau_comp[outside]).any() and \
       not np.all(tau_comp[outside] == nodata):
        raise SystemExit("FAIL-CLOSED: finite τ outside Ø100 m disk.")

    odir = pdir / "overlay"
    odir.mkdir(parents=True, exist_ok=True)
    tags = {"PATCH_ID": a.patch, "CONTRACT": CONTRACT,
            "COMPOSITE": spec.composite_rule, "WEIGHTS": spec.weight_method}
    write_geotiff(odir / "composite.tif", ach_comp, transform, 31983,
                  nodata, {**tags, "QUANTITY": "ACH_composite_per_h"})
    write_geotiff(odir / "composite_tau.tif", tau_comp, transform, 31983,
                  nodata, {**tags, "QUANTITY": "tau_composite_s"})
    write_geotiff(odir / "composite_qa_ndir.tif",
                  qa_count.astype(np.float32), transform, 31983, 0,
    ...
    print(f"    grid {shape} @ {TARGET_PX} m  EPSG:31983  "
          f"valid_px={int(good.sum())}")
    return 0
