...
Run:  ~/miniconda3/envs/MorphoFavela/bin/python scripts/cfd_overlay/build_qgis_project.py \
          --patch VDG-P07 [--repo-root ~/SCL/SCR/MorphoFavela]
"""
...
def patch_center(cx, cy, meta) -> gpd.GeoDataFrame:
    attrs = {k: [v] for k, v in meta.items() if isinstance(v, (int, float, str))}
    attrs["label"] = [
...
def hillshade(terrain: Path, out: Path) -> bool:
    ...
    try:
        ...
        print(
            f"  ! hillshade skipped ({type(e).__name__}); terrain layer still styled as elevation",
            file=sys.stderr,
        )
        return False
...
def qml_raster_pseudocolor(band_min, band_max, ramp, opacity=1.0, classes=12, label_suffix=""):
    stops = []
...
def ramp_div(f):  # diverging blue(low ACH/high τ, bad)→white→red(good)
    pts = [(0.0, (33, 102, 172)), (0.5, (247, 247, 247)), (1.0, (178, 24, 43))]
    return _lerp(pts, f)
...
def build(repo_root: Path, patch: str):
    ...
    layers.append(
        (
            "CFD output: τ / ACH composite (pending)",
            "../overlay/composite.tif",
            "gdal",
            "raster",
            qml_raster_pseudocolor(0, 1, ramp_div, opacity=0.6, label_suffix=" (rel)"),
        )
    )
    layers.append(
        ("Patch centre + meta", "layers/patch_center.gpkg", "ogr", "point", qml_center_point())
    )
    layers.append(
        (
            "Ø100 m analysis disk",
            "layers/analysis_disk.gpkg",
            "ogr",
            "polygon",
            qml_outline("255,127,0,255", 0.6),
        )
    )
    layers.append(
        (
            "CFD domain QC (indicative)",
            "layers/cfd_domain_qc.gpkg",
            "ogr",
            "polygon",
            qml_outline("120,120,120,200", 0.3, dashed=True),
        )
    )
    layers.append(
        (
            "Buildings (height; 0 flagged)",
            "../inputs/buildings.gpkg",
            "ogr",
            "polygon",
            qml_buildings(),
        )
    ...
    if have_hs:
        layers.append(
            (
                "Terrain hillshade",
                "layers/terrain_hillshade.tif",
                "gdal",
                "raster",
                qml_raster_gray(opacity=0.55),
            )
        )
    layers.append(
        (
            ...
            "gdal",
            "raster",
            qml_raster_pseudocolor(tmin, tmax, ramp_terra, label_suffix=" m"),
        )
    )
    ...
    print(
        f"    layers={len(layers)}  styles={len(layers)}  "
        f"hillshade={'yes' if have_hs else 'NO'}  crs=EPSG:{EPSG}"
    )
    print("    composite.tif is the only pending datasource (Phase 3)")
    return 0
...
def _qgs_xml(patch: str, layers) -> str:
    ...
    for i, (name, src, prov, gt, style) in enumerate(layers):
        ...
        if gt == "raster":
            body = f"<provider>{prov}</provider>{style}<blendMode>0</blendMode>"
            ltype = 'type="raster"'
        else:
