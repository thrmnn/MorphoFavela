...
def main():
    ...
    # ---- Extrusion validity ----
    skipped_height = kept_final[
        kept_final[args.height_field].isna() | (kept_final[args.height_field] <= 0)
    ] if args.height_field in kept_final.columns else gpd.GeoDataFrame()
    ...
    if len(removed_by_cluster) > 0:
        removed_by_cluster.plot(ax=ax, color="#f39c12", edgecolor="#d68910", linewidth=0.2, alpha=0.8)
    ...
    ax.set_title(f"Filter Overview\n{n_total:,} → {len(kept_final):,} "
                 f"({total_removed:,} removed, {total_removed/n_total*100:.1f}%)", fontsize=12)
    ax.set_aspect("equal")
    ...
    if len(removed_by_area) > 0:
        removed_by_area.plot(ax=ax, column="_footprint_area", cmap="Reds",
                             edgecolor="#c0392b", linewidth=0.5, alpha=0.9,
                             legend=True, legend_kwds={"shrink": 0.5, "label": "Area (m²)"})
        ...
        for _, row in top_removed.iterrows():
            ...
            ax.annotate(
                ...
                xy=(cx, cy), fontsize=6, color="darkred",
    ...
    if boundary_gdf is not None:
        boundary_gdf.boundary.plot(ax=ax, color="black", linewidth=1.5, linestyle="--")
    ax.set_title(f"Area-Filtered Buildings (>{MAX_FILTER_AREA} m²)\n"
                 f"{len(removed_by_area):,} removed — are these important?", fontsize=12)
    ax.set_aspect("equal")
    ax.tick_params(labelsize=7)
    ...
    kept_final.plot(ax=ax, color="#d5f5e3", edgecolor="#abebc6", linewidth=0.05, alpha=0.5)
    if len(removed_by_cluster) > 0:
        removed_by_cluster.plot(ax=ax, color="#f39c12", edgecolor="#d68910",
                                linewidth=0.5, alpha=0.9)
    roads_gdf.plot(ax=ax, color="#95a5a6", linewidth=0.3, alpha=0.3)
    if boundary_gdf is not None:
        boundary_gdf.boundary.plot(ax=ax, color="black", linewidth=1.5, linestyle="--")
    ax.set_title(f"Cluster-Filtered Buildings (buffer={BUILDING_CLUSTER_BUFFER}m)\n"
                 f"{len(removed_by_cluster):,} isolated buildings removed", fontsize=12)
    ax.set_aspect("equal")
    ax.tick_params(labelsize=7)
    ...
    if args.height_field in gdf.columns:
        ...
        h_removed_area = removed_by_area[args.height_field].dropna().values if len(removed_by_area) > 0 else np.array([])
        h_removed_area = h_removed_area[np.isfinite(h_removed_area) & (h_removed_area > 0)] if len(h_removed_area) > 0 else np.array([])

        bins = np.arange(0, max(h_kept.max() if len(h_kept) else 0,
                                h_removed_area.max() if len(h_removed_area) else 0) + 2, 1)

        ax.hist(h_kept, bins=bins, color="#2ecc71", alpha=0.7, label=f"Kept (n={len(h_kept):,})")
        if len(h_removed_area) > 0:
            ax.hist(h_removed_area, bins=bins, color="#e74c3c", alpha=0.7,
    ...
    if args.height_field in gdf.columns:
        ...
        ax2.axvline(MAX_FILTER_AREA, color="red", linewidth=2, linestyle="--",
        ...
        if removed_mask.sum() > 0:
            ax2.scatter(a_all[removed_mask], h_all[removed_mask], s=8, alpha=0.6,
                        c="#e74c3c", label=f"Removed ({removed_mask.sum():,})", zorder=5)

        ax2.set_xlabel("Footprint area (m²)", fontsize=12)
        ax2.set_ylabel(f"Height [{args.height_field}] (m)", fontsize=12)
        ax2.set_title(f"{area} — Footprint Area vs Height\n"
                      f"Red line = MAX_FILTER_AREA = {MAX_FILTER_AREA} m²", fontsize=13)
