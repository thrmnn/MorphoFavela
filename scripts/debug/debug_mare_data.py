...
def main():
    ...
    print(
        f"  Bounds:      ({dtm_bounds.left:.0f}, {dtm_bounds.bottom:.0f}) - "
        f"({dtm_bounds.right:.0f}, {dtm_bounds.top:.0f})"
    )
    print(
        f"  Extent:      {dtm_bounds.right - dtm_bounds.left:.0f} x "
    ...
    if len(valid_vals) > 0:
        print(
            f"  Elevation:   {valid_vals.min():.1f} - {valid_vals.max():.1f} m "
    ...
    if boundary_gdf is not None:
        ...
        print(
            f"  Buildings inside boundary: {inside.sum():,} / {len(fp_gdf):,} "
    ...
    if boundary_gdf is not None:
        legend_items.append(
            Patch(facecolor="none", edgecolor="black", linestyle="--", label="Boundary")
        )
    ...
    if height_field:
        ...
        if len(heights) > 0:
            ...
            ax.set_ylabel("Count")
            ax.set_title(
                f"Building Heights (n={len(heights):,})\n"
                f"range [{heights.min():.1f}, {heights.max():.1f}]m",
                fontsize=11,
            )
        else:
            ax.text(
    ...
    roads_plot.plot(
        ...
        alpha=0.9,
        legend=True,
        legend_kwds={"shrink": 0.6, "label": "Segment length (m)"},
    )
    if boundary_gdf is not None:
        boundary_gdf.boundary.plot(ax=ax, color="black", linewidth=1.5, linestyle="--")
    ax.set_title(
    ...
    if "base" in fp_gdf.columns:
        ...
        ax.set_ylabel("Count")
        ax.set_title(
            f"DTM Elevation at Building Centroids\n"
            f"No 'base' field; {n_outside:,}/{len(fp_gdf):,} outside DTM",
            fontsize=11,
        )
    ...
    ax.axvline(
        ...
        color="red",
        linestyle="--",
        label=f"Mean={np.mean(areas):.0f} m²",
    )
    ax.axvline(
        np.median(areas),
        color="orange",
        linestyle="--",
        label=f"Median={np.median(areas):.0f} m²",
    )
    # Mark outliers
    ...
    ax.axvline(
        ...
        color="purple",
        linestyle=":",
        label=f"P99={p99:.0f} m² ({n_outliers} above)",
    )
    ax.legend(fontsize=9)
    ax.set_xlabel("Footprint area (m²)")
    ax.set_ylabel("Count")
    ax.set_title(
        f"Building Footprint Areas (n={len(areas):,})\n"
        f"range [{areas.min():.0f}, {areas.max():.0f}] m²",
        fontsize=11,
    )
