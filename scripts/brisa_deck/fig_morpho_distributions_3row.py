#!/usr/bin/env python3
"""BRISA slide deck — fig_morpho_distributions_3row.png.

Three-row sibling of fig_morpho_distributions.py:
  Row 1: λf (frontal-area density, 8-direction mean)
  Row 2: SVF (sky view factor)
  Row 3: slope (degrees)

Each row: ridge / violin distributions for the 5 favelas
(hillside-first: Vidigal, Rocinha, Alemão, Rio das Pedras, Maré).
Hillside = solid black fill; flatland = hatched grey. Median marked.

Aspect ≈ 1.4 (slightly wider than tall). 200 DPI.

Output: /home/theo/brisa_paper/artifacts/slides/assets/fig_morpho_distributions_3row.png
"""
...
def ridge_row(ax, data: dict, *, xlim, xlabel, bw=0.22, fmt="{:.2f}", height=0.85):
    ...
    ax.set_yticklabels([SITE_LABELS[s] for s in SITES], fontsize=10.5)
    ax.invert_yaxis()
    ax.set_xlim(*xlim)
...
def main() -> None:
    ...
    gap = 0.055
    bot_y = 0.06
    mid_y = bot_y + row_h + gap
    top_y = mid_y + row_h + gap

    ax_top = fig.add_axes([left, top_y, row_w, row_h])  # λf
    ax_mid = fig.add_axes([left, mid_y, row_w, row_h])  # SVF
    ax_bot = fig.add_axes([left, bot_y, row_w, row_h])  # slope

    ridge_row(ax_top, lambdaf_data,
              xlim=(0.0, 10.0),
              xlabel=r"$\lambda_f$ (frontal-area density, 8-direction mean)",
              bw=0.22, fmt="{:.2f}")
    ridge_row(ax_mid, svf_data,
              xlim=(0.0, 1.0), xlabel="Sky View Factor",
              bw=0.22, fmt="{:.2f}")
    ridge_row(ax_bot, slope_data,
              xlim=(0.0, 45.0), xlabel="Slope (degrees)",
              bw=0.30, fmt="{:.1f}°")

    ax_top.set_title(r"Frontal-area density $\lambda_f$ — per favela",
                     loc="left", fontsize=12.5, pad=6, color="#111111")
    ax_mid.set_title("Sky View Factor — per favela",
                     loc="left", fontsize=12.5, pad=6, color="#111111")
    ax_bot.set_title("Slope — per favela",
                     loc="left", fontsize=12.5, pad=6, color="#111111")
    ...
    leg = fig.legend(handles=handles, loc="upper right",
                     ...
                     frameon=True, fontsize=9.5, title="Typology", title_fontsize=10,
                     handlelength=2.0)
    leg.get_frame().set_linewidth(0.4)
    leg.get_frame().set_edgecolor("#888888")
    leg.get_frame().set_facecolor("white")

    out = OUT_DIR / "fig_morpho_distributions_3row.png"
