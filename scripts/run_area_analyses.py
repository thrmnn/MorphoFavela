...
def main():
    ...
    parser.add_argument(
        "--skip-metrics", action="store_true", help="Skip basic metrics calculation"
    )
    parser.add_argument(
        "--grid-spacing",
        type=float,
    ...
    if not args.skip_metrics:
        cmd = [
            sys.executable,
            "scripts/calculate_morphology_metrics.py",
            "--area",
            area,
            "--basic-only",
        ]
    ...
    if roads_files:
        ...
        cmd = [
            sys.executable,
            "scripts/compute_svf_streets.py",
            "--stl",
            str(stl_file),
            "--roads",
            str(roads_file),
    ...
    cmd = [
        sys.executable,
        "scripts/compute_solar_access.py",
        "--stl",
        str(stl_file),
        "--footprints",
        str(footprints_file),
    ...
    cmd = [
        sys.executable,
        "scripts/compute_sectional_porosity.py",
        "--footprints",
        str(footprints_file),
        "--grid-spacing",
        "2.0",
    ...
    cmd = [
        sys.executable,
        "scripts/compute_occupancy_density.py",
        "--stl",
        str(stl_file),
        "--footprints",
        str(footprints_file),
    ...
    if svf_file.exists() and solar_file.exists() and porosity_file.exists():
        ...
        cmd = [
            sys.executable,
            "scripts/compute_deprivation_index_raster.py",
            "--solar",
            str(solar_file),
            "--svf",
            str(svf_file),
