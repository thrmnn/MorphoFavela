...
def generate_grid(bounds: tuple, grid_spacing: float) -> tuple:
    """
...
def compute_sectional_porosity(
    grid_cells: gpd.GeoDataFrame, footprints: gpd.GeoDataFrame, grid_spacing: float
) -> np.ndarray:
    """
    ...
    for idx, cell_row in tqdm(
        grid_cells.iterrows(), total=len(grid_cells), desc="  Computing porosity"
    ):
...
This is a geometric analysis based on building footprints only, NOT a CFD simulation.
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Grid cell spacing in meters (default: 2.0)",
    )
    parser.add_argument(
        "--height",
        type=float,
        default=1.5,
        help="Section height in meters (default: 1.5, pedestrian level)",
    )
    parser.add_argument(
        "--buffer",
        type=float,
        default=0.25,
        help="Footprint buffer distance in meters (default: 0.25)",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default=None,
        help="Output directory (default: outputs/porosity)",
    )
