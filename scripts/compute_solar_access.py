...
def generate_solar_maps(
    ...
    print(
        f"  Points with <{threshold_hours}h: {np.sum(solar_hours < threshold_hours)} ({np.sum(solar_hours < threshold_hours) / len(solar_hours) * 100:.1f}%)"
    )
    print(
        f"  Points with ≥{threshold_hours}h: {np.sum(solar_hours >= threshold_hours)} ({np.sum(solar_hours >= threshold_hours) / len(solar_hours) * 100:.1f}%)"
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.25,
        help="Buffer distance for building footprints (meters)",
    )
    parser.add_argument(
        "--building-buffer",
        type=float,
        default=None,
        help="Only compute solar access for points within this distance of buildings (meters). Reduces computation by focusing on relevant areas.",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default=None,
        help="Output directory (default: outputs/solar)",
    )
    parser.add_argument(
        "--latitude",
        type=float,
        default=-22.9519,
        help="Site latitude (degrees, default: Rio de Janeiro)",
    )
    parser.add_argument(
        "--longitude",
        type=float,
        default=-43.2105,
        help="Site longitude (degrees, default: Rio de Janeiro)",
    )
    parser.add_argument(
        "--timestep",
        type=int,
        default=60,
        help="Solar calculation time step in minutes (default: 60)",
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=2.0,
        help="Solar access threshold in hours (default: 2.0)",
    )
    ...
    if args.footprints:
        ...
        if not footprints_path.exists():
            ...
            building_footprints, isolated_footprints, area_filtered_footprints = (
                load_building_footprints(
                    footprints_path,
    ...
    sun_directions, sun_times = compute_sun_positions(
        latitude=args.latitude, longitude=args.longitude, timestep_minutes=args.timestep
    )
