...
def extract_building_height_at_point(
    ...
    if hasattr(building_footprint, "exterior"):
        ...
        closest_poly = min(
            ...
            if hasattr(building_footprint, "geoms")
            else [building_footprint],
...
def find_nearby_buildings(
    street_point: Point, buildings_gdf: gpd.GeoDataFrame, search_radius: float = 75.0
) -> list:
    """
...
def sample_street_points(
    roads_gdf: gpd.GeoDataFrame, spacing: float, terrain: pv.PolyData
) -> gpd.GeoDataFrame:
    """
    ...
    # Create GeoDataFrame
    points_gdf = gpd.GeoDataFrame(metadata, geometry=all_points, crs=roads_gdf.crs)
...
def create_street_exceedance_map(
    ...
    if selected_points_gdf is not None:
        ...
        labels_map = {
            "high": "High Exceedance",
            "mean": "Mean Exceedance",
            "low": "Low Exceedance",
        }
    ...
    ax.set_title(
        f"Street-Level Sky Exposure Plane Exceedance\n{ruleset_name} Ruleset",
        fontsize=14,
        fontweight="bold",
    )
    ax.set_aspect("equal")
...
def create_statistics_plots(
    ...
    if len(exceedance_data) > 0:
        ...
        ax.set_title(
            f"Distribution of Street-Level Exceedance Values\n{ruleset_name} Ruleset",
            fontsize=14,
            fontweight="bold",
        )
        ...
        ax.axvline(
            ...
            linestyle="--",
            linewidth=2,
            label=f"Median: {median_exceed:.2f}m",
        )
        ax.legend()
    else:
...
def create_street_section_views(
    ...
    mean_exceedance = (
        np.mean(exceedance_values[exceedance_values > 0])
        if np.any(exceedance_values > 0)
        else np.mean(exceedance_values)
    )
    ...
    logger.info(
        f"  Selected points: High={high_point['exceedance']:.2f}m, "
    ...
    for point_row, label, exceedance_value in selected_points:
                ...
                for building_idx, distance in building_distances:
                    ...
                    if actual_height > envelope_height:
                        ...
                        front_building_height = (
                            actual_height  # Store this building's height for exceedance
                        )
                        break
        ...
        valid_mask = (
            building_heights_array > terrain_z_array + 0.5
        )  # Buildings at least 0.5m above terrain
        ...
        terrain_base = min_terrain - max(
            5.0, terrain_range * 0.1
        )  # Base slightly below minimum terrain
        ...
        ax.plot(
            distances_array,
            envelope_heights_array,
            "r--",
            linewidth=2.5,
            label="Envelope (Allowed)",
            alpha=0.9,
        )
        ...
        exceedance_mask = has_building_at_point & (
            front_building_heights_array > envelope_heights_array
        )
        if np.any(exceedance_mask):
            ...
            exceedance_top = np.minimum(
                front_building_heights_array[exceedance_mask],
        ...
        if exceedance_value > 0:
            ...
            if not np.isnan(envelope_at_street) and not np.isnan(building_at_street):
                ...
                ax.plot(
                    [0, arrow_x],
                    [envelope_at_street, envelope_at_street],
                    "r:",
                    linewidth=1.5,
                    alpha=0.7,
                    zorder=24,
                ...
                ax.plot(
                    [0, arrow_x],
                    [building_at_street, building_at_street],
                    "k:",
                    linewidth=1.5,
                    alpha=0.7,
                    zorder=24,
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=3.0,
        help="Distance between sample points (meters, default: 3.0)",
    )
    parser.add_argument(
        "--search-radius",
        type=float,
        default=75.0,
        help="Search radius for nearby buildings (meters, default: 75.0)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Area name (e.g., vidigal_tls, copacabana) for automatic path resolution",
    )
    ...
    if roads_gdf is not None:
        ...
        points_with_exceedance = (exceedance_array > 0).sum()
        print(
            f"  Points with exceedance: {points_with_exceedance} ({points_with_exceedance / len(exceedance_array) * 100:.1f}%)"
        )
    ...
    print(
        f"  Buildings with exceedance: {buildings_with_exceedance} / {total_buildings} ({buildings_with_exceedance / total_buildings * 100:.1f}%)"
    )
