...
def draw_site_panel(ax, site: str, grid: gpd.GeoDataFrame, state: np.ndarray, panel: str) -> dict:
    ...
    classified.plot(ax=ax, column="state", cmap=cmap, norm=norm,
    ...
    ax.set_title(f"({panel}) {label}  ·  {typology}",
                 loc="left", fontsize=7.5, color="#222222", pad=2.5)
    ax.text(
        0.02, 0.04,
        ...
        transform=ax.transAxes, fontsize=5.0, color="#444444", va="bottom",
    )
    return shares
...
def draw_typology_panel(ax, per_site: dict[str, dict]) -> None:
    ...
    colors_for_states = {"adequate": "#FFFFFF", "sun_only": "#D9D9D9",
                         "vent_only": "#7F7F7F", "compound": "#111111"}
    edge = "#444444"
    for k, _ in STATE_LEGEND:
        vals = np.array([r[1][k] for r in rows]) * 100.0
        ax.barh(labels, vals, left=bottoms, color=colors_for_states[k],
                edgecolor=edge, linewidth=0.5, height=0.62)
        for i, v in enumerate(vals):
            if v >= 4:
                text_color = "#FFFFFF" if k == "compound" else "#222222"
                ax.text(bottoms[i] + v / 2, i, f"{v:.0f}%",
                        ha="center", va="center", fontsize=6.0, color=text_color)
        bottoms += vals
    ax.set_xlim(0, 100)
...
def draw_shared_legend(fig) -> None:
    ...
    for key, label in STATE_LEGEND:
        color = {"adequate": "#FFFFFF", "sun_only": "#D9D9D9",
                 "vent_only": "#7F7F7F", "compound": "#111111"}[key]
    ...
    fig.legend(handles=handles, loc="lower center", bbox_to_anchor=(0.5, -0.012),
               ncol=4, frameon=False, fontsize=6.5, handlelength=1.8,
...
def main() -> None:
    ...
    for panel, (gsspec, site) in axes_layout.items():
        ...
        n = int(sum((state == STATE_INT[k]).sum() for k in ("adequate", "sun_only", "vent_only", "compound")))
