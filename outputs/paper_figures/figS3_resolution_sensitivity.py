...
Canonical output (used in the manuscript):

    figS3_resolution_sensitivity.{svg,png}
        5 sites × 5 indicators, KDE curves 10m vs 20m overlaid. Shows that
        10m resolution captures structural features (bimodality in SVF,
        lambda_f detail) that 20m aggregation smooths away.

Supplementary variants (in exports/_variants/):

    figS3b_scatter_comparison.{svg,png}   — Vidigal upscale-vs-native scatter
    figS3c_difference_map.{svg,png}       — Vidigal difference maps

The canonical figure is produced when this script is run. The variants
can be regenerated by passing --variants on the command line.
"""
...
def make_canonical():
    ...
    src_png = EXPORTS_DIR / "figS3a_distribution_overlay.png"
    dst_svg = EXPORTS_DIR / "figS3_resolution_sensitivity.svg"
    dst_png = EXPORTS_DIR / "figS3_resolution_sensitivity.png"
    if src_svg.exists():
...
def main():
    ...
    print("Generating canonical Fig S3 (distribution overlay)...")
    make_canonical()

    if args.variants:
        print("Generating supplementary variants...")
        make_variant_b()
        make_variant_c()
