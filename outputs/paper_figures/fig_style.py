...
def add_scalebar(ax, length_m: float | None = None, loc: str = "lower left") -> None:
    ...
    ax.add_patch(plt.Rectangle(
        ...
        facecolor="k", edgecolor="k", linewidth=0.3, zorder=20, clip_on=False,
    ))

    label = f"{length_m / 1000:.0f} km" if length_m >= 1000 else f"{length_m:.0f} m"
    ax.text(
        x0 + length_m / 2, y0 + bar_h + ey * 0.006, label,
        ha="center", va="bottom", fontsize=5, zorder=20, clip_on=False,
    )
...
def add_north_arrow(ax, loc: str = "upper right", size: float = 0.04) -> None:
    ...
    ax.annotate(
        "", xy=(cx, base_y + arrow_len), xytext=(cx, base_y),
        arrowprops=dict(arrowstyle="-|>", color="k", lw=0.8),
        zorder=20,
    )
    ax.text(
        cx, base_y + arrow_len + ey * 0.008, "N",
        ha="center", va="bottom", fontsize=5, fontweight="bold", zorder=20,
    )
...
def load_campaign_patches(site: str):
    ...
    if not path.exists():
        # Fallback to pilot
        path = PROJECT_ROOT / "outputs" / site / "sampling_cfd" / "pilot_sampling" / "pilot_patches.csv"
    if not path.exists():
